    ...
    def _load_course_data(self) -> Dict:
        ...
        for course in courses:
            ...
            for skill in course_skills:
                skill_tasks = Task.objects.filter(skills=skill)
                for task in skill_tasks:
                    # Поскольку задание может быть связано с несколькими навыками,
                    # мы добавляем отдельную запись для каждого навыка
                    course_tasks.append({
                        'id': task.id,
                        'skill_id': skill.id,
